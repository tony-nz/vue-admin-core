<template>
  <div :class="flex" class="flex gap-2">
    ...
    <div v-if="resource.lock && showDefaults && canAction('update')">
      <button
        @click="$emit('lockPopup', { $event, data })"
        v-tooltip="'Lock ' + resource.singularName.toLowerCase()"
        :class="btnLockClass"
      >
        <svg
          class="h-4 w-4"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const btnLockClass = computed(() => {
      return "transition duration-150 ease-in-out btn bg-gray-100 border-gray-800 rounded-lg fill-gray-400 hover:bg-orange-300 hover:fill-white dark:bg-slate-800 p-2 shadow";
    });
